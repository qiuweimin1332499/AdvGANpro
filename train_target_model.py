...
if __name__ == "__main__":
    ...
    extractor, input_size = feature_extract_net(target_name,
                                                num_features,
                                                feature_extract_only,
                                                use_pretrained).initialize_model()
    extractor = extractor.to(device)  # 除非forward否则精确写出到函数才能to(device)
    extractor.train()  # set the model in train mode
    opt_model = torch.optim.Adam(extractor.parameters(), lr=0.001)
    epochs = 120
    for epoch in range(epochs):
        loss_epoch = 0
        if epoch == 80:
            opt_model = torch.optim.Adam(extractor.parameters(), lr=0.0001)
        for i, data in enumerate(train_dataloader, 0):
            train_imgs, train_labels = data
            train_imgs, train_labels = train_imgs.to(device), train_labels.to(device)
            logits_model = extractor(train_imgs)
            loss_model = F.cross_entropy(logits_model, train_labels)
            loss_epoch += loss_model
            opt_model.zero_grad()
            loss_model.backward()
        ...
        print('loss in epoch %d: %f' % (epoch, loss_epoch))
    # save model
    feature_extract_file_name = './feature_extract.pth'
    torch.save(extractor.state_dict(), feature_extract_file_name)
    ...
    for i, data in enumerate(test_dataloader, 0):
        test_img, test_label = data
        test_img, test_label = test_img.to(device), test_label.to(device)
        pred_lab = torch.argmax(extractor(test_img), 1)
        num_correct += torch.sum(pred_lab==test_label,0)
    num_correct=num_correct.cuda().data.cpu().numpy()  # num_correct先转为cpu变量再转为numpy变量
    print('accuracy in testing set: %f\n'%(num_correct/len(test_dataset)))
